...
class Storage:
    ...
    def _init(self):
        with self._conn() as c:
            ...
            c.execute("CREATE TABLE IF NOT EXISTS feature_snapshots (id INTEGER PRIMARY KEY AUTOINCREMENT, market_id TEXT, created_at TEXT, payload TEXT)")
    ...
    def research_history(self,market_id:str,limit:int=100)->dict:
        ...
        with self._conn() as c:
            preds=c.execute("SELECT created_at,payload FROM predictions WHERE market_id=? ORDER BY id DESC LIMIT ?",(market_id,n)).fetchall()
            feats=c.execute("SELECT created_at,payload FROM feature_snapshots WHERE market_id=? ORDER BY id DESC LIMIT ?",(market_id,n)).fetchall()
        def decode(rows):
            out=[]
            for r in rows:
    ...
    def calibration_pairs(self)->tuple[list[float],list[int]]:
        sql="""SELECT p.payload,r.outcome FROM predictions p JOIN resolutions r ON r.market_id=p.market_id JOIN (SELECT market_id,MAX(id) latest_id FROM predictions GROUP BY market_id) latest ON latest.latest_id=p.id ORDER BY p.id"""
        probs=[]; outcomes=[]
        with self._conn() as c:
    ...
    def stats(self)->dict:
        with self._conn() as c:
            names=("market_snapshots","feature_snapshots","predictions","evidence_snapshots","paper_trades","live_trades","resolutions","trader_profiles","experiments")
            counts={name:int(c.execute(f"SELECT COUNT(*) FROM {name}").fetchone()[0]) for name in names}
        return counts
    ...
    def live_notional_since(self,since_iso:str,market_id:str|None=None)->float:
        with self._conn() as c:
            if market_id is None: row=c.execute("SELECT COALESCE(SUM(notional),0) FROM live_trades WHERE created_at>=?",(since_iso,)).fetchone()
            else: row=c.execute("SELECT COALESCE(SUM(notional),0) FROM live_trades WHERE created_at>=? AND market_id=?",(since_iso,market_id)).fetchone()
