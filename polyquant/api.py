...
from .models import BacktestRequest,ExperimentRequest,LiveOrderRequest,PaperOrderRequest,ResolveMarketRequest
from .service import QuantService

ROOT=Path(__file__).resolve().parent.parent
settings=get_settings();service=QuantService(settings);backtester=Backtester();cross_market=CrossMarketAnalyzer();experiments=ExperimentRegistry(service.storage,backtester);auto_trader=AutoTrader(service,settings.auto_interval_seconds,settings.auto_order_notional,settings.auto_max_trades_per_cycle)
app=FastAPI(title="PolyQuant Intelligence",version="3.0.0")

@app.get("/api/health")
async def health():return {"ok":True,"mode":settings.mode,"data_source":service.last_source,"live_execution":settings.live_execution_enabled,"event_feed":"external" if settings.event_feed_url else "demo","version":"3.0.0"}
...
async def demo_backtest():
    req=BacktestRequest(points=[{"price":.40,"model_probability":.49},{"price":.42,"model_probability":.52},{"price":.46,"model_probability":.54},{"price":.51,"model_probability":.55},{"price":.56,"model_probability":.56},{"price":.59,"model_probability":.56},{"price":.57,"model_probability":.54}],position_pct=.12,min_edge=.05);return backtester.run(req)
