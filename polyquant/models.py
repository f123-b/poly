...
class Opportunity(BaseModel):market:Market;features:FeatureSnapshot;prediction:Prediction;prediction_id:int|None=None
class RiskDecision(BaseModel):approved:bool;reasons:list[str]=Field(default_factory=list);max_notional:float=0.0;suggested_notional:float=0.0;decision_id:str|None=None
...
class PaperSettlement(BaseModel):id:str;market_id:str;outcome:Literal['YES','NO'];realized_pnl:float=0.0;cash_payout:float=0.0;created_at:datetime=Field(default_factory=lambda:datetime.now(timezone.utc))
...
class BacktestResult(BaseModel):starting_cash:float;ending_equity:float;roi:float;max_drawdown:float;trades:int;win_rate:float;sharpe:float;equity_curve:list[float];fees_paid:float=0.0;slippage_cost:float=0.0;turnover:float=0.0;partial_fills:int=0
