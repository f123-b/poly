...
class SmartMoneyClient:
    ...
    async def profile(self,wallet:str)->dict:
        ...
        positions,closed,activity,value=await asyncio.gather(self._get("/positions",{"user":wallet,"limit":500}),self._get("/closed-positions",{"user":wallet,"limit":50,"sortBy":"TIMESTAMP","sortDirection":"DESC"}),self._get("/activity",{"user":wallet,"limit":200}),self._get("/value",{"user":wallet}))
        ...
        top_categories=sorted(categories.items(),key=lambda kv:kv[1],reverse=True)[:5];position_value=sum(_num(x.get("value")) for x in value) if isinstance(value,list) else _num(value.get("value")) if isinstance(value,dict) else 0.0
    ...
    async def market_flow(self,condition_id:str,leaderboard_size:int=50,trade_limit:int=500)->dict:
        if self.demo or condition_id.startswith("demo-"):return self._demo_flow(condition_id or "demo")
        if not re.match(r"^0x[a-fA-F0-9]{64}$",condition_id):raise ValueError("invalid condition id")
        leaders,trades=await asyncio.gather(self.leaderboard("OVERALL","ALL",leaderboard_size),self._get("/trades",{"market":condition_id,"limit":min(max(trade_limit,1),10000)}));ranks={str(x.get("wallet") or "").lower():x for x in leaders};gross=0.0;net_yes=0.0;matched=0
        for t in trades:
        ...
        score=net_yes/gross if gross else 0.0;return {"condition_id":condition_id,"smart_trades":matched,"gross_notional":gross,"net_yes_notional":net_yes,"score":max(-1.0,min(1.0,score)),"leaderboard_sample":len(leaders)}
    @staticmethod
    ...
    def _demo_flow(condition_id:str):
        seed=sum(ord(c) for c in condition_id);score=((seed%41)-20)/100;return {"condition_id":condition_id,"smart_trades":18,"gross_notional":82400.0,"net_yes_notional":82400.0*score,"score":score,"leaderboard_sample":3,"demo":True}
